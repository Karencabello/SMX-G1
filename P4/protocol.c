...
void run_sink(){
    ...
    for(s = 0; s < nstns; s++){
        if(stns[s].qu.lng >= MAXQU)
            ERROR((ofile,"%d QUEUE Length (%d) of stn %d larger than MAXQU(%d). Increment MAXQU", slot, 
    ...
    for (s = 0; s < nstns; s++){
        ...
        for(i = 0; i < STSSTATES; i++){                 
            ...
            if(sts.snt[i][s] != d)                          
                ERROR((ofile,"%d ERROR CHECK STN %d state %d: pk sent %d != samples in delay histogram  %d",             
                        slot, i, sts.snt[i][s], d));   
        }    
